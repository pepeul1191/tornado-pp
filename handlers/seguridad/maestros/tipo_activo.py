...
class SeguridadTipoActivosHandler(BaseHandler):
    def get(self):
		helper = Helper()
		if self.get_secure_cookie("usuario") or helper.get('ambiente') != 'produccion':
			...
			self.render("seguridad/maestros/tipo_activos.html", helper = helper)
		else:
			self.redirect('/login')

class SeguridadTipoActivosListarHandler(BaseHandler):
    def get(self):
		helper = Helper()
		if self.get_secure_cookie("usuario") or helper.get('ambiente') != 'produccion':
...
class SeguridadTipoActivosGuardarHandler(BaseHandler):
	def post(self):
		helper = Helper()
		if self.get_secure_cookie("usuario") or helper.get('ambiente') != 'produccion':
